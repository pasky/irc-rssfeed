...
#print "[$rss_url] Fetched XML data, encoding ".$rss->encoding.":\n";
  ...
  foreach my $item (@{$rss->{items}}) {
    ...
    $item->{creator} = encode('utf8', $item->{dc}->{creator}); # if (uc($rss->encoding) eq 'UTF-8');
    ...
    if (length $item->{description} > 33) {
	    substr($item->{description}, 30, 9999, '...');
    }

    $item->{link} =~ s#^https?://.*?/#https://brmlab.cz/# unless $item->{link} =~ m#^https?://soup#;

    #print "-- Item: $item->{title} / $item->{creator} :: $item->{link}\n$item->{description}\n--\n\n";
...
sub delta_rss {
  ...
  my @delta = grep { not exists $seen->{$_->{link}}; } @$new;
  foreach (@delta) {
    $seen->{$_->{link}} = 1;
    #print "<".$_->{title}.">\n";
  }
...
# }
  if (@new_items) {
    ...
    foreach my $item (reverse @delta) {
      if ($extract_url) {
	my $link = $item->{link};
	$link =~ s/^.*[?&]url=//; # XXX: We rely on it being the last parameter.
	$item->{link} = uri_unescape($link);
      }
      $conn->privmsg ($channel, $rss_name . ($item->{creator} ? "[" . $item->{creator} . "] " : "") . "\2" . $item->{title}."\2 \3"."14::\3 ".$item->{link});
